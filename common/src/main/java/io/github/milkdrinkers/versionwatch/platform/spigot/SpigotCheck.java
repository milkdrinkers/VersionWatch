...
public class SpigotCheck implements PlatformImplementation {
    ...
    public @Nullable Version parseResponse(final InputStream inputStream) throws BadResponseException {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream, StandardCharsets.UTF_8))) {
            final JsonObject versionData = JsonParser.parseReader(reader).getAsJsonObject();

            final String version = versionData.getAsJsonPrimitive("name").getAsString().toUpperCase();
